...
class StarSiphonEffectImpl implements StarSiphonEffect {
    private static final ParticleParams PARTICLE_PARAMS = ParticleParams.create()
            ...
            .withSizeRange(Range.of(8.0f, 24.0f))
    ...
    public void renderFlash() {
        ...
        final Vector2f consumerFlashLocation = new Vector2f(consumerLocation);
    ...
    private void enterInState() {
        ...
        final ParticleSystem particleSystem = getParticleSystem();
        particleSystem.setEmitterLocation(locationDifference);
        particleSystem.clearRepulsors();
    ...
    private void advanceActiveState(float amount) {
        ...
        particleSystem.addAttractor("attractor", consumerLocation,  finalAttractorStrength * forceMultiplier, 3000f);
        //particleSystem.addRepulsor("repulsor1",  consumerLocation, 25f * forceMultiplier, consumerRadius * 25.0f);
        //particleSystem.addRepulsor("repulsor2",  consumerLocation, 200f * forceMultiplier, consumerRadius * 2.0f);
    ...
    private void renderLayer(
        ...
        final Vector2f locationDifference = Vector2f.sub(consumerLocation, starLocation, null);
        final float distance = locationDifference.length() - (starRadius + consumerRadius);
        locationDifference.normalise();
        locationDifference.scale(starRadius);
        ...
        locationDifference.normalise();
        locationDifference.scale(starRadius + consumerRadius + (distance * Math.min(1.0f, 0.5f + 25.0f * EasingFunctions.easeOutSine(intensity))));
        final Vector2f endPosition = Vector2f.add(starLocation, locationDifference, null);
